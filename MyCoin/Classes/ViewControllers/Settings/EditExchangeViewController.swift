...
class EditExchangeViewController: UIViewController {
    ...
    var exchangeId: NSManagedObjectID?
...
// MARK:- Events
extension EditExchangeViewController {
...
extension EditExchangeViewController {
    ...
    private func exchangeNameError() -> Bool {
        return (exchangeName.count > 20 || exchangeName.isEmpty)
    }
    
    private func exchangeAddressError() -> Bool {
        if let address = exchangeAddress {
    ...
    private func saveExchangeInfo() {
        let loading = LoadingView.view()
        
        if false == exchangeNameError(), false == exchangeAddressError() {
            var success = false
            
            if let id = exchangeId {
                success = MyExchange.updateExchange(name: exchangeName, remarks: exchangeAddress, byId: id)
            } else {
                success = MyExchange.addExchange(name: exchangeName, remarks: exchangeAddress)
            }
...
extension EditExchangeViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            ...
            cell.stringValue = exchangeName
            let error = exchangeNameError()
            cell.error = (valid ? error : false)
            cell.didChangeInputValue = { [weak self] (value) in
                if let strongSelf = self, let v = value {
                    strongSelf.exchangeName = v
                }
            }
            return cell
        case 1:
            let cell = tableView.dequeueReusableNibCell(with: TextViewCell.self) as! TextViewCell
            cell.value = exchangeAddress
            cell.error = exchangeAddressError()
            cell.didChangeInputValue = { [weak self] (value) in
                if let strongSelf = self, let v = value {
                    strongSelf.exchangeAddress = v
                }
            }
            return cell
        default:
...
extension EditExchangeViewController: UITableViewDelegate {
